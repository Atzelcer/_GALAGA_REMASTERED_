...
AHUD_P_Principal::AHUD_P_Principal()
{
    ...
    //Para el sonido de transcion
    static ConstructorHelpers::FObjectFinder<USoundBase> IntroPrincipalSoundObject(TEXT("SoundWave'/Game/Audio_Unreal_Engine/GALAGA_SOUND_MUSIC/TRANSICION_PORTAL.TRANSICION_PORTAL'"));
    if (IntroPrincipalSoundObject.Succeeded())
	{
        RootTransion = IntroPrincipalSoundObject.Object;
	}
    ...
    HistoryAudioComponent = CreateDefaultSubobject<UAudioComponent>(TEXT("HistoryAudioComponent"));
    HistoryAudioComponent->bAutoActivate = false;
    HistoryAudioComponent->SetupAttachment(RootComponent);

    // Cargar los audios de la historia
    static ConstructorHelpers::FObjectFinder<USoundBase> HistorySound1(TEXT("/Game/HISTORY_GALAGA/HISTORY_SOUNDS/historia_0.historia_0"));
    static ConstructorHelpers::FObjectFinder<USoundBase> HistorySound2(TEXT("/Game/HISTORY_GALAGA/HISTORY_SOUNDS/historia01.historia01"));
    static ConstructorHelpers::FObjectFinder<USoundBase> HistorySound3(TEXT("/Game/HISTORY_GALAGA/HISTORY_SOUNDS/historia02.historia02"));
    static ConstructorHelpers::FObjectFinder<USoundBase> HistorySound4(TEXT("/Game/HISTORY_GALAGA/HISTORY_SOUNDS/historia03.historia03"));
    static ConstructorHelpers::FObjectFinder<USoundBase> HistorySound5(TEXT("/Game/HISTORY_GALAGA/HISTORY_SOUNDS/Historia04.Historia04"));
    static ConstructorHelpers::FObjectFinder<USoundBase> HistorySound6(TEXT("/Game/HISTORY_GALAGA/HISTORY_SOUNDS/historia05.historia05"));

    if (HistorySound1.Succeeded()) HistorySounds.Add(HistorySound1.Object);
    if (HistorySound2.Succeeded()) HistorySounds.Add(HistorySound2.Object);
    if (HistorySound3.Succeeded()) HistorySounds.Add(HistorySound3.Object);
    if (HistorySound4.Succeeded()) HistorySounds.Add(HistorySound4.Object);
    if (HistorySound5.Succeeded()) HistorySounds.Add(HistorySound5.Object);
    if (HistorySound6.Succeeded()) HistorySounds.Add(HistorySound6.Object);


    //Iniciando el audio de transicion
    AudioComponentTransion = CreateDefaultSubobject<UAudioComponent>(TEXT("AudioComponentTransion"));
    AudioComponentTransion->bAutoActivate = false;
    AudioComponentTransion->SetupAttachment(RootComponent);
...
void AHUD_P_Principal::PlayNextHistoryAudio()
{
    if (HistorySounds.IsValidIndex(CurrentAudioIndex))
    {
        if (HistoryAudioComponent)
        {
            HistoryAudioComponent->SetSound(HistorySounds[CurrentAudioIndex]);
            HistoryAudioComponent->Play();
            CurrentAudioIndex++;
        }
    }
    else
...
void AHUD_P_Principal::StopHistoryAudio()
{
    if (HistoryAudioComponent)
    {
        HistoryAudioComponent->Stop();
    }
}

void AHUD_P_Principal::OnContinueHistoryClicked()
{
    PlayNextHistoryAudio();
}

void AHUD_P_Principal::OnSkipHistoryClicked()
{
    StopHistoryAudio();
}
...
void AHUD_P_Principal::ShowTransitionScreen()
{
    if (TransitionScreenClass)
    {
        ...
        if (TransitionScreen)
        {
            ...
            if (RootTransion && AudioComponentTransion)
            {
                AudioComponentTransion->SetSound(RootTransion);
                AudioComponentTransion->Play();
            }
            APlayerController* PlayerController = GetWorld()->GetFirstPlayerController();
            if (PlayerController)
            {
...
void AHUD_P_Principal::HideTransitionScreen()
{
    if (TransitionScreen)
    {
        if (AudioComponentTransion)
        {
            AudioComponentTransion->Stop();
        }
...
void AHUD_P_Principal::ShowMissionMessage(const FString& MissionMessage)
{
    if (MissionMessageClass) 
    {
        ...
        {
            // Agrega el widget al viewport y actualiza el mensaje
